...
function AddApp() {
  ...
  const onFinish = async (values) => {
    console.log(!!!otherForm.category.length);
    if (!!!otherForm.image || !!!otherForm.image || !!!otherForm.category.length) {
      !!!otherForm.image &&
        setOtheFormError((prev) => ({
          ...prev,
          image: "Không để trống trường này",
        }));
      !!!otherForm.icon &&
        setOtheFormError((prev) => ({
          ...prev,
          icon: "Không để trống trường này",
        }));
      !!!otherForm.category.length &&
        setOtheFormError((prev) => ({
          ...prev,
          category: "Không để trống trường này",
        }));
    } else {
