...
function SelectPageSizeMultiple({
  ...
  const handleClick = (valueClick) => {
    ...
    if (IndexArr === -1) {
      ...
      onClick(value.filter((elemt) => elemt["_id"] !== valueClick["_id"]));
    }
  };
          ...
          {!isLoading ? (
            children.map((item, index) => (
              <Option
                value={item}
                active={
                  value.findIndex((elemt) => elemt["_id"] === item["_id"]) !==
                  -1
                }
              ...
              >
                <div className="Option_label">
                  <p className="Option_text">
                    {item.name}
                    {value.findIndex(
                      (elemt) => elemt["_id"] === item["_id"]
                    ) !== -1 && <CheckOutlined className="Option_icon" />}
                  </p>
                </div>
              </Option>
            ))
